function importAccountBalances() {
    $('.uploader_txt').text('');
    $('.errors').addClass('hidden');
    $('.errors_list').empty();
    var formData = new FormData();
    var file_data = $('#upload_account_balance')[0].files[0];
    ...
    $.ajax({
        url: $('#import_account_balance_form').attr('action'),
        type: 'POST',
        data: formData,
        dataType: "json",
        cache: false,
        ...
        success: function () {
            ...
            $('#import_account_balance_form')[0].reset(); // this will reset the form fields
        }
    });
}
