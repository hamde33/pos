...
(function(window, document, $) {
  ...
  $.app.menu = {
        ...
        switch (currentBreakpoint.name) {
          ...
          case 'lg':
            if(menuType === 'vertical-overlay-menu'){
                ...
                this.collapseExpand(currentBreakpoint.name)
              // this.hide();
            }
            else{
                if(!localStorage.getItem("collapsed")) {
    ...
    collapseMenu: function(screenSize){
      var menuType = $body.data('menu');
      switch (screenSize) {
          case 'xl':
          case 'lg':
    ...
    expandMenu: function(screenSize){
      var menuType = $body.data('menu');
      switch (screenSize) {
          case 'xl':
          case 'lg':
    ...
    collapseExpand: function(screenSize) {
      if(localStorage.getItem("collapsed") === "true"){
         this.collapseMenu(screenSize);
      }else{
          this.expandMenu(screenSize);
      }

        setTimeout(function() {
            if($('#dashboard_steps_done').val() === '0'
                && $('#user_is_company_owner').val() === '1') {
                $("#dashboard_popover").popover({content: popoverContent, html: true});
                $("#dashboard_popover").popover('show');
                $('.popover-content').parent('.popover').addClass('col-lg-6 col-md-6 col-sm-6 col-xs-8');
            }
        }, 1000);
    },
    ...
    changeMenu: function(screen){
      ...
      toggleCollapseExpandStatus: function() {
          if (localStorage.getItem("collapsed") === "true") {
              // toggle was on, turning it off
              localStorage.setItem("collapsed", "false");
          } else if (localStorage.getItem("collapsed") === "false") {
    ...
    toggle: function() {
      ...
      var menu = $body.data('menu');
      this.toggleCollapseExpandStatus();
      switch (currentBreakpoint.name) {
        case 'xl':
        case 'lg':
        case 'md':
        ...
        // $("#dashboard_popover").popover({content: popoverContent, html: true});
        $("#dashboard_popover").popover('hide');
        setTimeout(function() {
            if($('#dashboard_steps_done').val() === '0'
                && $('#user_is_company_owner').val() === '1') {
                $("#dashboard_popover").popover('show');
  ...
  $.app.nav = {
      ...
      $('.navigation-main').on('mouseenter.app.menu', 'li', function() {
        ...
        if( $body.hasClass('menu-collapsed') ){
          ...
          if($this.css( "border-top" )){
            fromTop = $this.position().top + parseInt($this.css("border-top"), 10) + parseInt("13px", 10);
          }
          else{
            fromTop = $this.position().top + parseInt("13px", 10);
          }
    ...
    adjustSubmenu: function ( $menuItem ) {
      ...
      topPos = menutop + $menuItem.height() + borderWidth  + parseInt("14px", 10);
